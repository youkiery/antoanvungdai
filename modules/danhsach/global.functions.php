...
function selectPetidOfOwner($userid) {
  ...
  $list = array();
  $sql = 'select id from `'. PREFIX .'_sendinfo` where userid = ' . $userid;
  $query = $db->query($sql);
...
function updatePet($data, $id) {
  ...
  $sql = 'update `'. PREFIX .'_sendinfo` set ' . implode(', ', $sql_part) . ' where id = ' . $id;
...
function checkPetOwner($petid, $userid) {
  ...
  $sql = 'select * from `'. PREFIX .'_sendinfo` where id = "'. $petid .'" and userid = ' . $userid;
  $query = $db->query($sql);
...
function checkPet($name, $userid) {
  ...
  $sql = 'select * from `'. PREFIX .'_sendinfo` where name = "'. $name .'" and userid = ' . $userid;
  $query = $db->query($sql);
...
function getPetDeactiveList($keyword = '', $page = 1, $limit = 10) {
  ...
  $sql = 'select count(*) as count from `'. PREFIX .'_sendinfo` where name like "%'. $keyword .'%" or microchip like "%'.$keyword.'%" and active = 0';
  $query = $db->query($sql);
  $data['count'] = $query->fetch()['count'];

  $sql = 'select * from `'. PREFIX .'_sendinfo` where name like "%'. $keyword .'%" or microchip like "%'.$keyword.'%" and active = 0 limit ' . $limit . ' offset ' . (($page - 1) * $limit);
  $query = $db->query($sql);
