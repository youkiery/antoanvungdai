...
function listContent() {
  ...
  $sql = 'select count(*) as count from `'. PREFIX .'_sendinfo` a inner join `'. PREFIX .'_contact` b on a.owner = b.id where a.active = 1 and (a.name like "%'.$filter['keyword'].'%" or a.micro like "%'.$filter['keyword'].'%" or b.fullname like "%'.$filter['keyword'].'%" or b.mobile like "%'.$filter['keyword'].'%")';
  $query = $db->query($sql);
  $data['count'] = $query->fetch()['count'];
  $count = $data['count'];
  
  $sql = 'select a.* from `'. PREFIX .'_sendinfo` a inner join `'. PREFIX .'_contact` b on a.owner = b.id where a.active = 1 and (a.name like "%'.$filter['keyword'].'%" or a.micro like "%'.$filter['keyword'].'%" or b.fullname like "%'.$filter['keyword'].'%" or b.mobile like "%'.$filter['keyword'].'%") order by a.time desc limit ' . $filter['limit'] . ' offset ' . ($filter['page'] - 1) * $filter['limit'];
  $query = $db->query($sql);
