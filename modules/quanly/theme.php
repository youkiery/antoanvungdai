...
function danhsachgiong() {
  ...
  $sql = "select * from ". PREFIX ."_danhmuc_giong where kichhoat = 1 order by ten asc";
  $danhsach = $db->all($sql);

  foreach ($danhsach as $giong) {
    $xtpl->assign('id', $giong['id']);
    $xtpl->assign('ten', $giong['ten']);
...
function danhsachthanhvien() {
  ...
  foreach ($danhsach as $user) {
    ...
    $xtpl->assign('tinhtranghoatdong', ($user['active'] ? 'Bình thường' : 'Bị cấm'));
